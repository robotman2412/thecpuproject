...
public class Expression {
    ...
    public static long[] readValue(String labelPrefix, String token, Pass1Out pass1, int index, long bitmask) {
        if (token.charAt(0) == '.') {
            if (pass1.labels.containsKey(labelPrefix + token)) {
            ...
            try {
                ...
                throw new CompilerSyntaxError(pass1.tokensSourceFiles[index], index, e.getMessage(), e);
            }
        }
        ...
        else if (token.startsWith("0q") || token.startsWith("0o")) {
            try {
        ...
        else if (token.endsWith("q") || token.endsWith("o")) {
            try {
        ...
        throw new CompilerSyntaxError(pass1.tokensSourceFiles[index], index, "Value literal format invalid.");
    }
